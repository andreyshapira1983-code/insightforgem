...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Utility to build storage key per channel
  function storageKey(channel) {
    return 'starGalaxyChat_' + channel;
  }

  // Load messages for a channel from localStorage
  function loadMessages(channel) {
    let msgs = [];
    try {
      msgs = JSON.parse(localStorage.getItem(storageKey(channel))) || [];
    } catch (e) {
      msgs = [];
    }
    return msgs;
  ...
  function saveMessages(channel, msgs) {
    localStorage.setItem(storageKey(channel), JSON.stringify(msgs));
  }
