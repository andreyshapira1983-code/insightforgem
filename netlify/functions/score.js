...
const MODEL_CHAT = 'gpt-4o';                 // безопаснее, чем 'gpt-4o-mini'
...
async function openai(path, body, apiKey){
  ...
  if (!r.ok) {
    const err = new Error(data?.error?.message || text || `OpenAI ${r.status}`);
    err.status = r.status;
    throw err;
  }
  return data || {};
...
exports.handler = async (event) => {
  try {
    ...
    // БЕРЁМ ЛЮБОЙ ИЗ ТВОИХ КЛЮЧЕЙ (в таком порядке)
    const apiKey =
    ...
    if (!apiKey) {
      return { statusCode: 500, body: JSON.stringify({ error:'No API key in env vars' }) };
    }
    ...
    // 1) Оригинальность через эмбеддинги
